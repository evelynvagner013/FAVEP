    ...
    switch (this.reportType) {
      case 'productivity':
        ...
        filteredProducoes.forEach(prod => {
          ...
          const propArea = this.propriedades.find(p => String(p.id) === String(prod.propriedadeId))?.area || 0;
        ...
        scales: chartType !== 'pie' ? {
          y: {
            ...
            ticks: {
              ...
              // A verificação extra foi removida, pois já estamos dentro
              // de um bloco que só executa quando chartType não é 'pie'.
              callback: (value: any) => {
